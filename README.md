...
基于 GitHub Actions 的零成本自动化投研邮件系统，聚焦 **AI / 半导体赛道**，把两件最容易拖延的事自动化：

1. **回调择时** —— 盯盘 6 个半导体核心标的，达到预设回调档位时按纪律提醒分批建仓
2. **价投跟踪** —— 季度抓取 5 位价值投资大师的 SEC 13F 持仓，diff 出新建/清仓/加减仓，并找出多人共识标的

无服务器、无数据库，纯 Python 脚本 + 定时 workflow，状态以 JSON 形式回写仓库持久化。
...
├── superinvestors_monitor.py        # 管线 2: 大师 13F 监控 (SEC EDGAR)
├── alert_state.json                 # 管线 1 状态: 警报冷却、周报时间
├── superinvestor_state.json         # 管线 2 状态: 上季持仓、accession 号
└── .github/workflows/
    ├── monitor.yml                  # 工作日美东收盘后跑 ETF 监控
    └── superinvestors.yml           # 13F 公布期日跑 / 其余时间周一跑
```
...
GitHub Actions 跑完后，workflow 会自动把 `alert_state.json` / `superinvestor_state.json` 用 `[skip ci]` commit 回主分支，下次运行时读回。

- `alert_state.json` 记录：每个 ticker_档位 的最后警报时间、上次周报时间、首次运行日期
- `superinvestor_state.json` 记录：每位大师的最近 accession 号、上一季持仓快照（用于 diff）
...
python monitor.py
python superinvestors_monitor.py
```
